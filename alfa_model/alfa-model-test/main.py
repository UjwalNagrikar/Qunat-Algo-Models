...
"""
================================================================================
  NSE FUTSTK BACKTESTING ENGINE  v4.0
  Strategy : Cross-Sectional Mean Reversion  |  Futures (FUTSTK) Mechanics
  Universe : 12 NSE Large-Cap Stocks  |  2010 - 2024

  TWO-PHASE DESIGN  (strict IS / OOS separation)
  ─────────────────────────────────────────────────────────────────────────────
  IN-SAMPLE   (IS)  2010-01-01 to 2021-12-31
    Optimisation : 2010-2017  (8 years)  <- grid-search all param combinations
    Validation   : 2018-2021  (4 years)  <- score top-N candidates, pick winner

  OUT-OF-SAMPLE (OOS) 2023-01-01 to 2024-12-31  (2 years, NEVER SEEN in IS)
    Apply LOCKED best params, no re-fitting, pure unseen-data test

  KEY PRINCIPLE
  The Optimisation window defines the strategy.
  The Validation window selects the param set that holds OOS-of-optimisation.
  The OOS window is a pure forward-test -- parameters are frozen before it starts.

  OUTPUT  ->  Two clean printed sheets only, no plots, no trade logs
================================================================================
"""
...
DATES = {
    ...
    "data_end":   "2024-12-31",
    "opt_start":  "2010-01-01",   # Optimisation window start
    "opt_end":    "2017-12-31",   # Optimisation window end   (8 years)
    "val_start":  "2018-01-01",   # Validation window start
    "val_end":    "2021-12-31",   # Validation window end     (4 years)
    "oos_start":  "2023-01-01",   # OOS start -- UNSEEN during IS
    "oos_end":    "2024-12-31",   # OOS end   (2 years)
}
...
CFG = {
    ...
    "param_grid": {
        ...
        "z_threshold":    [0.75, 1.0, 1.5, 2.0],
        "vol_threshold":  [0.020, 0.025, 0.035],
        "cooldown_days":  [3, 5],
    },
    ...
    "stop_loss_pct":    None,
    "n_top_candidates": 10,
    # Futures cost model (NSE / Zerodha)
    "brokerage_flat": 20.0,
...
def load_data(cfg):
    ...
    return data, ("Yahoo Finance (live)" if live else
                  "Synthetic (negative-AC AR1, NSE price levels 2010)")


# ==============================================================================
#  FEATURES  (computed once on full history)
# ==============================================================================
def build_features(data, cfg):
    ...
    def _csz(df):
        mu = df.mean(axis=1); sig = df.std(axis=1).replace(0, np.nan)
...
def build_signals(features, params, s_date, e_date):
    ...
    rnk = _w(features["cs_rank"]); zc  = _w(features["z_comp"])
    v10 = _w(features["vol10"]);   cl  = _w(features["close"])
    ma  = _w(features["ma"])
    lp  = params.get("long_pct",  0.25)
    sp  = params.get("short_pct", 0.25)
    vt  = params.get("vol_threshold", 0.025)
    zt  = params.get("z_threshold",   1.0)
    tf  = params.get("use_trend_filter", True)
    hi  = v10 > vt
    rnk2 = rnk.where(~hi); zc2 = zc.where(~hi)
    lc = (rnk2 <= lp)       & (zc2 <= -zt)
    sc = (rnk2 >= (1 - sp)) & (zc2 >=  zt)
    if tf:
        lc = lc & (cl > ma).fillna(False)
        sc = sc & (cl < ma).fillna(False)
    sig = lc.fillna(False).astype(np.int8) - sc.fillna(False).astype(np.int8)
    return sig, zc2.abs()
...
def _trec(tid, sym, pos, dates, i, rsn, xp, gp, cost):
    ...
    return {"trade_id":tid,"symbol":sym,
            ...
            "gross_pnl":round(gp,2),"cost":round(cost,2),"net_pnl":round(net,2),
...
def run_backtest(features, signals, ret_score, params, s_date, e_date):
    ...
    op=_w(features["open"]); cp=_w(features["close"])
    sig=_w(signals); rs=_w(ret_score)
    dates    = op.index.tolist()
    free_cap = float(params["initial_capital"])
    ...
    for i, date in enumerate(dates):
        ...
        if sl is not None:
            for sym in list(positions):
                ...
                if tr < -sl:
                    ...
                    free_cap+=pos["margin"]+gp-cost; cooldown[sym]=i
        ...
        for sym,direction in cands.items():
            ...
            nl=max(1,int(alloc/mpl)); tot_mg=nl*mpl
            while tot_mg>cur_eq*mm and nl>1: nl-=1; tot_mg=nl*mpl
            if tot_mg>free_cap*0.93:
                nl=max(1,int(free_cap*0.90/mpl)); tot_mg=nl*mpl
            if tot_mg>free_cap or nl<1: continue
            free_cap-=tot_mg
            positions[sym]={"i":i,"ep":ep,"nl":nl,"lot":lot,"dir":int(direction),"margin":tot_mg}
...
def compute_metrics(tdf, edf, params, label=""):
    init=params["initial_capital"]; eq=edf["equity"]
    ...
    stp=(tdf[tdf["exit_reason"]=="STOP"] if "exit_reason" in tdf.columns
         else pd.DataFrame())
    return {
        "label":label,"total_ret_pct":rp,"total_ret_inr":ri,"cagr":cagr,
...
def run_optimisation(features, cfg):
    pg=cfg["param_grid"]; keys=list(pg.keys())
    combos=list(itertools.product(*[pg[k] for k in keys]))
    s,e=DATES["opt_start"],DATES["opt_end"]; results=[]
    ...
    for vals in combos:
        ...
        try:
            ...
            row.update({"sharpe_opt":round(m["sharpe"],4),
                        ...
                        "net_pnl_opt":round(m["total_ret_inr"],0)})
            results.append(row)
...
# ==============================================================================
#  VALIDATION SCORING  (2018-2021)
# ==============================================================================
def run_validation(features, cfg, top_candidates):
    s,e=DATES["val_start"],DATES["val_end"]; scored=[]
    pg_keys=list(cfg["param_grid"].keys())

    for cand in top_candidates:
        p={**cfg,**{k:v for k,v in cand.items() if k in pg_keys},
        ...
        try:
            ...
            row.update({"sharpe_opt":cand.get("sharpe_opt",0),
    ...
    if not scored:
        fallback={**cfg,"holding_period":3,"z_threshold":1.0,
                  "vol_threshold":0.025,"cooldown_days":3,"stop_loss_pct":None}
...
def print_sheet1(opt_df, val_df, best_params, is_m, val_m, cfg, src):
    ...
    print(); print(SEP)
    print("  SHEET 1  |  IN-SAMPLE  |  NSE FUTSTK ENGINE v4.0  |  Universe Capital")
    print(SEP)
    ...
    print(f"  Capital         : INR {cfg['initial_capital']:,.0f}  |  "
          f"Margin: {cfg['margin_pct']*100:.0f}%  |  "
          f"Risk/Trade: {cfg['risk_pct']*100:.1f}%  |  "
          f"Max Positions: {cfg['max_positions']}")
    print(f"  Signal          : Composite z-score = avg(z_1day + z_3day)")
    print(f"  Filters         : Vol filter (10d rolling std)  |  "
          f"Trend filter (20d MA)")
    print(f"  Futures Sizing  : Risk budget / margin_per_lot -> integer lots")
    print(f"  PnL Formula     : (Exit - Entry) x Lot Size x Num Lots x Direction")
    ...
    print(f"\n  Grid size: {len(opt_df):,} combinations | "
          f"Param keys: {list(cfg['param_grid'].keys())}")
    ...
    print(f"  {'Rk':>2}  {'Hold':>4}  {'Z-Thr':>5}  {'Vol%':>4}  {'CD':>2}"
          ...
          f"  {'WR%':>5}  {'PF':>6}  {'Trades':>6}  {'Cost%':>5}  {'NetPnL(L)':>10}")
    print("  " + "."*80)
    for i, row in opt_df.head(15).iterrows():
        pnl_l = row["net_pnl_opt"]/100000
    ...
    print(f"\n  C. VALIDATION SCORING  (2018-2021)  |  Top-N re-tested to select winner")
    print(SEP2)
    print(f"  Combined Score = sqrt(IS_Sharpe x Val_Sharpe)  "
          f"[geometric mean -- penalises IS overfitting]")
    print()
    print(f"  {'Rk':>2}  {'Hold':>4}  {'Z-Thr':>5}  {'Vol%':>4}  {'CD':>2}"
          f"  {'IS_Sh':>6}  {'V_Sh':>6}  {'Comb':>6}"
          f"  {'VCAGR%':>7}  {'VDD%':>6}  {'VWR%':>5}  {'VPF':>6}  {'Trades':>6}")
    print("  " + "."*80)
    for i, row in val_df.iterrows():
        ...
        print(f"  {i+1:>2}  {int(row['holding_period']):>4}  "
              f"{row['z_threshold']:>5.2f}  {row['vol_threshold']*100:>4.1f}  "
              f"{int(row['cooldown_days']):>2}"
              f"  {row['sharpe_opt']:>6.4f}  {row['sharpe_val']:>6.4f}  "
              f"{row['combined_score']:>6.4f}"
              f"  {row['cagr_val']:>7.2f}  {row['max_dd_val']:>6.2f}"
              f"  {row['wr_val']:>5.1f}  {row['pf_val']:>6.4f}"
              f"  {int(row['n_trades_val']):>6}{marker}")

    # D. Locked Parameters
    print(f"\n  D. LOCKED OPTIMAL PARAMETERS  (frozen -- never changed)")
    print(SEP2)
    print(f"  Holding Period    = {int(best_params['holding_period'])} days")
    print(f"  Z-Score Threshold = {best_params['z_threshold']:.2f} sigma  "
          f"(|composite z| > threshold to enter)")
    print(f"  Vol Filter        = {best_params['vol_threshold']*100:.1f}%  "
          f"(skip stocks with 10d rolling vol above this)")
    print(f"  Cooldown          = {int(best_params['cooldown_days'])} days  "
          f"(no re-entry on same stock within window)")
    print(f"  Trend Filter      = 20-day MA  "
          f"(LONG above MA, SHORT below MA)")
    print(f"  Stop-Loss         = Disabled  "
          f"(time-exit only -- mean reversion needs the bounce)")
    print(f"  Max Positions     = {cfg['max_positions']}")

    # E. IS Performance
    print(f"\n  E. IN-SAMPLE PERFORMANCE  (best params, per sub-window)")
    print(SEP2)
    print(f"  {'Metric':<34}{'Opt 2010-17':>20}{'Val 2018-21':>20}")
    print("  " + "-"*74)
    ...
    _row("Total Trades",        fn(g(is_m,"n_trades")),       fn(g(val_m,"n_trades")))
...
def print_sheet2(oos_m, best_params, is_m, val_m, src, runtime):
    ...
    print(); print(SEP)
    print("  SHEET 2  |  OUT-OF-SAMPLE  |  NSE FUTSTK ENGINE v4.0  |  Universe Capital")
    print("  PURE FORWARD TEST  |  Parameters frozen before OOS window opened")
    print(SEP)
    ...
    print(f"  Data Status     : UNSEEN -- zero contact with IS optimisation or validation")
    print(f"  Params          : LOCKED from IS validation scoring, not modified")
    print()
    print(f"  LOCKED PARAMS (reminder):")
    print(f"    Holding Period    = {int(best_params['holding_period'])} days")
    print(f"    Z-Score Threshold = {best_params['z_threshold']:.2f} sigma")
    print(f"    Vol Filter        = {best_params['vol_threshold']*100:.1f}%")
    print(f"    Cooldown          = {int(best_params['cooldown_days'])} days")
    print(f"    Stop-Loss         = Disabled  |  Trend Filter = 20d MA")
    print(f"    Data Source       = {src}")

    # B. OOS Performance
    print(f"\n  B. OUT-OF-SAMPLE PERFORMANCE  (2023-2024)")
    print(SEP2)
    print(f"  {'Metric':<40}{'Value':>34}")
    print("  " + "-"*74)
    ...
    _row("Total Trades",                   fn(g("n_trades")))
    ...
    print(f"\n  C. IS vs OOS COMPARISON  (regime robustness check)")
    print(SEP2)
    ...
    _crow("Net PnL",
          f"INR {is_m.get('total_ret_inr',0)/1e5:.2f}L",
          f"INR {val_m.get('total_ret_inr',0)/1e5:.2f}L",
          f"INR {(g('total_ret_inr') or 0)/1e5:.2f}L")
    ...
    deg_ov  = (sh_opt-sh_val)/(abs(sh_opt)+1e-9)*100
    deg_vo  = (sh_val-sh_oos)/(abs(sh_val)+1e-9)*100

    print(f"  Sharpe path: IS-Opt {sh_opt:.4f}  ->  IS-Val {sh_val:.4f}"
          f"  ->  OOS {sh_oos:.4f}")
    print(f"  Degradation: IS-Opt to IS-Val = {deg_ov:>+.1f}%  |  "
          f"IS-Val to OOS = {deg_vo:>+.1f}%")
    print()
    ...
    if sh_oos > 0.8 and pf_oos > 1.2:
        ...
        verdict = "SOLID     -- Positive OOS PnL, acceptable risk-adjusted return."
    elif sh_oos > 0 and pf_oos > 1.0:
        verdict = "MARGINAL  -- Profitable OOS but weak Sharpe. Monitor live."
    else:
        verdict = "WEAK      -- Strategy does not hold on OOS data. Revisit alpha."

    fit_check = ("Overfitting LOW" if abs(deg_ov) < 40
                 else "OVERFITTING RISK -- IS-Opt Sharpe inflated vs Val")
    cost_check = (f"Cost/Gross = {ctg:.1f}% -- "
                  + ("healthy" if ctg < 50 else "HIGH -- edge too thin"))

    print(f"  Verdict    : {verdict}")
    print(f"  Fit check  : {fit_check}")
    print(f"  Cost check : {cost_check}")
    print()
    print(f"  Lot Sizes  : " +
          "  ".join(f"{k}={v}" for k,v in FUTSTK_LOTS.items()))
...
def main():
    ...
    # 1. Load 2010-2024 data once
    data, src = load_data(cfg)

    # 2. Build features on full history (slice per window at signal time)
    features = build_features(data, cfg)

    # 3. Grid search on Optimisation window (2010-2017 only)
    opt_df, top_candidates = run_optimisation(features, cfg)

    # 4. Re-test top-N on Validation window (2018-2021), pick winner
    val_df, best_params = run_validation(features, cfg, top_candidates)

    # 5. Compute IS performance (Opt window with best params)
    s_o, e_o = DATES["opt_start"], DATES["opt_end"]
    sigs_o, sc_o = build_signals(features, best_params, s_o, e_o)
    tdf_o, edf_o = run_backtest(features, sigs_o, sc_o, best_params, s_o, e_o)
    is_m = compute_metrics(tdf_o, edf_o, best_params, "OPT")

    # 6. Compute Val performance (Val window with best params)
    s_v, e_v = DATES["val_start"], DATES["val_end"]
    sigs_v, sc_v = build_signals(features, best_params, s_v, e_v)
    tdf_v, edf_v = run_backtest(features, sigs_v, sc_v, best_params, s_v, e_v)
    val_m = compute_metrics(tdf_v, edf_v, best_params, "VAL")

    # 7. OOS test -- params LOCKED, completely fresh window 2023-2024
    s_oo, e_oo = DATES["oos_start"], DATES["oos_end"]
    sigs_oo, sc_oo = build_signals(features, best_params, s_oo, e_oo)
    tdf_oo, edf_oo = run_backtest(features, sigs_oo, sc_oo, best_params, s_oo, e_oo)

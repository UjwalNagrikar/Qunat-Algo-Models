...
#   Data Source: Local CSV (nifty_futures_continuous.csv)
# ═══════════════════════════════════════════════════════════════════════════════
...
CSV_FILE_PATH = "nifty_futures_continuous.csv"  # <--- YOUR CSV FILE
...
def load_csv_data(csv_path: str, start: str, end: str) -> pd.DataFrame:
    """
    Loads historical Nifty data from a local CSV file, standardizes columns, 
    and slices the requested timeframe.
    """
    if not os.path.exists(csv_path):
        raise FileNotFoundError(f"CRITICAL: Cannot find data file '{csv_path}'. Please make sure it is in the same directory.")
    
    print(f"  Reading {csv_path}...")
    df = pd.read_csv(csv_path)
    
    # 1. Auto-detect and parse the Date column
    date_cols = [c for c in df.columns if 'date' in c.lower() or 'time' in c.lower()]
    date_col = date_cols[0] if date_cols else df.columns[0]
    
    # Allow robust date parsing
    df[date_col] = pd.to_datetime(df[date_col], format='mixed', dayfirst=True)
    df.set_index(date_col, inplace=True)
    df.index.name = "Date"
    
    # 2. Standardize Column Names (e.g., 'open' -> 'Open', 'Vol' -> 'Volume')
    rename_map = {}
    for c in df.columns:
        cl = c.lower()
        ...
        elif cl in ['close', 'c', 'ltp']: rename_map[c] = 'Close'
    ...
    for req in required:
        if req not in df.columns:
            raise ValueError(f"Missing required column: {req} in your CSV")
            
    if 'Volume' not in df.columns:
        df['Volume'] = 100000  # Fallback fake volume if not present
        
    required.append('Volume')
    
    # 4. Sort, clean, and slice by requested dates
    df.sort_index(inplace=True)
    df = df[~df.index.duplicated(keep='last')] # Remove duplicate timestamps if any
...
def optimize(df_train: pd.DataFrame, df_test: pd.DataFrame,
    ...
    for ci, combo in enumerate(combos):
        ...
        try:
            ...
            full_years=yr_day_span[yr_day_span>=200].index
            if len(full_years)>0:
                yr_counts_full=tdf_te_tmp[tdf_te_tmp["yr"].isin(full_years)].groupby("yr").size()
                if yr_counts_full.min()<70: continue   

            # Composite score 
            score=(0.4*m_tr["sharpe"] + 0.4*m_tr["calmar"] + 0.2*m_te["sharpe"])
            results.append({**p,
...
╚══════════════════════════════════════════════════════════════╝""")
    ...
    print("\n[1/6] Loading historical data from CSV...")
    df_tr = load_csv_data(CSV_FILE_PATH, TRAIN_START, TRAIN_END)
    df_te = load_csv_data(CSV_FILE_PATH, TEST_START, TEST_END)
    
    print(f"  Train Set : {len(df_tr)} days  {df_tr.index[0].date()} → {df_tr.index[-1].date()}")
    print(f"  Test Set  : {len(df_te)} days  {df_te.index[0].date()} → {df_te.index[-1].date()}")

    print("\n[2/6] Running Optimization...")
    ...
    # ── UNSEEN TEST ONLY ──────────────────────────────────────────────────
    print_all_trades(tdf_te,"UNSEEN TEST DATA")
    print_metrics_report(m_te,"UNSEEN TEST PERFORMANCE")
    print_yearly_breakdown(trd_te, eq_te, test_cap)
    print_mc(mc)

    # ── Charts: test + MC only (no train chart) ───────────────────────────
    chart_main(se,tdf_te,eq_te,m_te,mc,"Unseen Test")
    chart_mc_deep(mc)
